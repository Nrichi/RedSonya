...
class Shop_ItemsController extends Zend_Controller_Action
{
...
public function indexAction()
    {
    	...
    	if ($input->isValid()) {
    		$q = Doctrine_Query::create()
    			...
    			->where('i.sold = ?', '0')
    			->addwhere('i.category_id != ?', '5');	//выбираем изделия, а не материалы;
    		...
    		$pagerUrlBase = $this->view->url(array(), 'shop-items', 1) . "/{%page}";
	...
	public function displayAction()
    {
    	...
    	//если передан правильный идентификатор поста, получаем товар из бд и передаем результат в вид
    	if ($input->isValid()) {
    		$q = Doctrine_Query::create()
    			->from('Redsonya_Model_Items i')
    		...
    		//выбираем награды и участие в выставках
    		$q = Doctrine_Query::create()
