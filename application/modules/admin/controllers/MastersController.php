<?php
class Admin_MastersController extends Zend_Controller_Action
{
	...
	public function createAction()
	{
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid($postData)) {
				...
				$item = new Redsonya_Model_Masters();
				...
				$newName = 'master'.time().'.'.$ext;		//генерируем уникальное имя для загруженной картинки
				...
				$item->master_id = NULL;					//айдишник автоинкремент, поэтому указываем его как null
				$item->img = $newName;
				$item->save();								//сохраняем строку в базе
		
				$this->_helper->getHelper('FlashMessenger')->addMessage('Новый сотрудник был успешно добавлен.');
				$this->_redirect('/admin/masters/index');
			}
		}
	}
	...
	{
		$form = new Redsonya_Form_MasterUpdate();
		$this->view->form = $form;
		...
		if ($this->getRequest()->isPost()) {
			...
			if ($form->isValid($postData)) {		
				...
				$input = $form->getValues();				//получаем данные из формы
				$item = Doctrine::getTable('Redsonya_Model_Masters')->find($input['master_id']);	//находим строку по id-шнику, которую нужно обновить
				...
				$this->_helper->getHelper('FlashMessenger')->addMessage('Информация о сотруднике была успешно обновлена.');
				$this->_redirect('/admin/masters/index');
			}
		} else {
			// if GET request
	...
	public function deleteAction()
	{
		if ($this->getRequest()->isPost()) {
			...
			$item = Doctrine::getTable('Redsonya_Model_Masters')->find($del);
			$oldImgName = $item['img'];					//сохраняем имя старой фотки
				
			//удаляем запись из базы
			$q = Doctrine_Query::create()
            	->delete('Redsonya_Model_Masters m')
            	->where('m.master_id = ?', $del);		
			$result = $q->execute();

			unlink(realpath(dirname('.')).DIRECTORY_SEPARATOR.'foto'.DIRECTORY_SEPARATOR.$oldImgName);
		
			$this->_helper->getHelper('FlashMessenger')->addMessage('Сотрудник был успешно удален');
			$this->_redirect('/admin/masters/index');
		} 
		else
		{
